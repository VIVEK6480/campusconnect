...
function getFacultyId(request: NextRequest): string | null {
  ...
  try {
    ...
    console.error("FACULTY PASSWORD JWT ERROR:", error);
    return null;
  }
}
  ...
  try {
    ...
    if (!facultyId) {
      return NextResponse.json(
        { success: false, message: "Unauthorized faculty account." },
        { status: 401 }
      );
    }
    ...
    if (!currentPassword || !newPassword || !confirmPassword) {
      return NextResponse.json(
        { success: false, message: "Current password and new password fields are required." },
        { status: 400 }
      );
    }
    ...
    const samePassword = await bcrypt.compare(newPassword, faculty.password);
    ...
    return NextResponse.json(
      { success: false, message: "Unable to update faculty password." },
      { status: 500 }
    );
  }
}
