...
function getLocalDateKey(value?: string | null): string {
  if (!value) {
    return "";
  }
...
function normalizeStatus(status?: string | null): string {
  return String(status || "Absent").trim().toUpperCase();
}
...
function getStoredStudent(): StudentUser {
  ...
  const keys = ["user", "student", "studentUser", "currentStudent"];
...
function normalizeRegisteredSubjects(data: AttendanceResponse): Subject[] {
  const result = new Map<string, Subject>();

  for (const subject of safeArray<Subject>(data.registeredSubjects)) {
  ...
  for (const registration of safeArray<Registration>(data.registrations)) {
  ...
  const attendanceRecords = safeArray<ClassAttendanceRecord>(
    data.classAttendance || data.attendance
  );
...
export default function StudentAttendancePage() {
  ...
  const [eventAttendance, setEventAttendance] = useState<EventAttendanceRecord[]>([]);
  ...
  const loadAttendance = useCallback(async (showRefresh = false) => {
    if (showRefresh) {
      setRefreshing(true);
    } else {
      setLoading(true);
    ...
    try {
      ...
      if (!response.ok || data.success === false) {
        throw new Error(data.message || "Unable to load student attendance.");
      }
      ...
      const subjects = normalizeRegisteredSubjects(data);
      const records = safeArray<ClassAttendanceRecord>(
        data.classAttendance || data.attendance
      );
      const eventRecords = safeArray<EventAttendanceRecord>(data.eventAttendance);

      setRegisteredSubjects(subjects);
      setClassAttendance(records);
      setEventAttendance(eventRecords);

      setSelectedEventId((current) => {
        if (current && eventRecords.some((record) => record.eventId === current)) {
          return current;
        }
      ...
      setError(
        err instanceof Error ? err.message : "Unable to load student attendance."
      );
      setClassAttendance([]);
  ...
  const selectedEvent = useMemo(() => {
    ...
    return eventAttendance.find((record) => record.eventId === selectedEventId) || null;
  ...
  const selectedSubject = useMemo(() => {
    ...
    return registeredSubjects.find((subject) => subject.id === selectedSubjectId) || null;
  }, [registeredSubjects, selectedSubjectId]);
  ...
  const selectedSubjectRecords = useMemo(() => {
    ...
    return classAttendance
      ...
      .sort((a, b) => {
        const aDate = new Date(a.session?.sessionDate || a.markedAt).getTime();
        const bDate = new Date(b.session?.sessionDate || b.markedAt).getTime();
        return bDate - aDate;
      });
  }, [classAttendance, selectedSubjectId]);

  const filteredSubjectRecords = useMemo(() => {
    const searchValue = search.trim().toLowerCase();
    return selectedSubjectRecords.filter((record) => {
      ...
      const subjectName = record.subject?.name || selectedSubject?.name || "";
      ...
      const matchesSearch =
        ...
        sessionDate.toLowerCase().includes(searchValue);
      return matchesDate && matchesStatus && matchesSearch;
    });
  }, [selectedSubjectRecords, selectedSubject, dateFilter, statusFilter, search]);

  const filteredStats = useMemo(() => {
    const total = filteredSubjectRecords.length;
    const present = filteredSubjectRecords.filter(
      (record) => normalizeStatus(record.status) === "PRESENT"
    ).length;
    const absent = filteredSubjectRecords.filter(
      (record) => normalizeStatus(record.status) === "ABSENT"
    ).length;
    const late = filteredSubjectRecords.filter(
          ...
          <section className="w-full px-5 py-7 sm:px-7 lg:px-8 xl:px-9">
              ...
              <div className="relative z-10 flex flex-col justify-between gap-7 lg:flex-row lg:items-center">
                <div>
                  ...
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                    View your own subject-wise attendance, present classes, absent classes and complete attendance history.
                  </p>
                </div>
                ...
                <div className="hidden h-24 w-24 shrink-0 items-center justify-center rounded-3xl border border-emerald-300/10 bg-emerald-300/10 lg:flex">
                  <ClipboardCheck size={46} className="text-emerald-300" />
                </div>
              </div>
            </div>
            ...
            <div className="mb-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <AttendanceStatCard
                ...
                title="Present"
                value={String(overallStats.present)}
              ...
              <AttendanceStatCard
                ...
                title="Absent"
                value={String(overallStats.absent)}
            ...
            {loading && (
              <div className="mb-7 flex min-h-[320px] items-center justify-center rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-col items-center gap-3 text-slate-400">
                  ...
                  <p className="text-sm">Loading your attendance...</p>
                </div>
              </div>
            )}
                  ...
                  <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        Only subjects registered by you are shown here.
                      </p>
                    </div>
                ...
                {registeredSubjects.length === 0 ? (
                  <div className="px-6 py-16 text-center sm:px-10">
                    ...
                    <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">
                      No subject registration is currently available for your account.
                    </p>
                  </div>
                ) : (
                  <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {registeredSubjects.map((subject) => {
                      const subjectRecords = classAttendance.filter(
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            window.setTimeout(() => {
                              document
                                .getElementById("selected-subject-attendance")
                                ?.scrollIntoView({
                                  behavior: "smooth",
                                  block: "start",
                                });
                        ...
                        >
                          ...
                          <div className="mt-5 flex items-end justify-between">
                            ...
                            <div className="text-right text-[11px]">
                              <p className="font-semibold text-emerald-600">P {present}</p>
                              <p className="font-semibold text-red-500">A {absent}</p>
                            </div>
                          </div>
              ...
              >
                <div className="border-b border-slate-100 bg-gradient-to-br from-white to-emerald-50/40 px-6 py-7 sm:px-8">
                  <div className="flex flex-col justify-between gap-6 xl:flex-row xl:items-center">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          Semester {selectedSubject.semester} • Your personal attendance history
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <MiniStat
                      label="Present"
                      value={String(filteredStats.present)}
                      icon={<CheckCircle2 size={17} />}
                      tone="green"
                    />
                    <MiniStat
                      label="Absent"
                      value={String(filteredStats.absent)}
                ...
                <div className="border-b border-slate-100 px-6 py-5 sm:px-8">
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-500">
                      {filteredSubjectRecords.length} records
                    </span>
                  </div>
                  ...
                  <div className="grid gap-3 lg:grid-cols-[1.3fr_0.8fr_0.8fr_auto]">
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={(event) =>
                          setStatusFilter(
                            event.target.value as "ALL" | "PRESENT" | "ABSENT"
                          )
                        }
                            ...
                            {filteredSubjectRecords.map((record) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-5 py-5">
                                    <p className="text-sm font-semibold text-slate-800">
                                      {record.subject?.name || selectedSubject.name}
                                    </p>
                                    <p className="mt-0.5 text-xs text-slate-400">
                    ...
                    <MiniStat
                      ...
                      value={`${
                        eventAttendance.length > 0
                          ? Math.round(
                              (overallStats.eventPresent / overallStats.eventTotal) * 100
                            )
                          : 0
                      }%`}
                      icon={<Trophy size={17} />}
                ...
                <div className="border-b border-slate-100 bg-gradient-to-br from-white to-blue-50/40 px-6 py-7 sm:px-8">
                  ...
                  <div className="mt-7 grid gap-3 sm:grid-cols-3">
                    <MiniStat
                      label="Status"
                      value={
                        normalizeStatus(selectedEvent.status) === "PRESENT"
                          ? "Present"
                          : "Absent"
                      }
                      icon={
                    ...
                    <MiniStat
                      label="Event Date"
                      value={formatDate(
                        selectedEvent.event?.eventDate || selectedEvent.markedAt
                      )}
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-5 py-5 text-sm text-slate-600">
                                  {formatDate(
                                    record.event?.eventDate || record.markedAt
                                  )}
                                </td>
                                <td className="px-5 py-5 text-sm text-slate-600">
                                  {record.event?.venue || "—"}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-emerald-900">
                      Your attendance is private
                    </p>
                    <p className="mt-1 text-xs leading-5 text-emerald-700">
                      This page displays only your own registered subjects and your own attendance records. Attendance is marked by faculty and is read-only for students.
                    </p>
                  </div>
                </div>
              </div>
...
function AttendanceStatCard({
  ...
  value: string;
  description: string;
  type: "attendance" | "present" | "absent" | "total";
  percentage?: number;
}) {
  let iconClass = "bg-emerald-50 text-emerald-500";
  if (type === "absent") {
...
function MiniStat({
  ...
  let wrapper = "border-slate-200 bg-white";
  let iconClass = "bg-slate-50 text-slate-500";
  ...
  if (tone === "blue") {
    ...
    iconClass = "bg-white text-blue-500";
    valueClass = "text-blue-600";
  }

  return (
    <div className={`rounded-2xl border p-4 ${wrapper}`}>
