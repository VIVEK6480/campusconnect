...
export default function FacultyEventsPage() {
  ...
  // Top-level cursor interaction for the complete event card.
  // The actual event image is never transformed or modified.
      ...
      try {
        ...
        for (const key of possibleKeys) {
          ...
          try {
            ...
            // Continue to the next possible localStorage key.
          }
        }
      } catch {
  ...
  async function handleDeleteEvent(eventId: string, title: string) {
    ...
    try {
      ...
      setSuccess("Event deleted successfully from CampusConnect database.");
    } catch (deleteError) {
  ...
  async function handleSignOut() {
    ...
    try {
      ...
      console.error("Local storage cleanup error:", storageError);
    }
      ...
      <div className="min-h-screen w-full min-w-0 lg:pl-[270px]">
        ...
        <main className="relative min-h-[calc(100vh-86px)] w-full overflow-hidden bg-[#edf4fa] px-4 py-6 sm:px-6">
          ...
          <div className="relative w-full">
            <section className="relative overflow-hidden rounded-[24px] border border-[#263951] bg-gradient-to-br from-[#0d1728] via-[#101d30] to-[#14273b] px-7 py-7 shadow-[0_18px_45px_rgba(10,27,48,0.18)] sm:px-9 lg:px-10">
              <div className="pointer-events-none absolute -right-20 -top-24 h-72 w-72 rounded-full border border-[#54bce5]/20" />
              <div className="pointer-events-none absolute right-8 top-14 h-40 w-40 rounded-full border border-[#54bce5]/10" />
              ...
              <div className="relative z-10 max-w-[1000px]">
                ...
                <h1 className="font-serif text-[35px] font-bold leading-[1.02] tracking-[-0.03em] text-white sm:text-[45px] lg:text-[51px]">
                  Create campus
                  <br />
                  <span className="text-[#69c9ed]">events & activities.</span>
                </h1>

                <p className="mt-4 max-w-[900px] text-[13px] leading-6 text-[#a7b7c9] sm:text-[14px]">
                  Create, manage and remove campus events directly from the
                  Faculty Portal. Changes are reflected in the same database
                  used by the Student Events section.
                </p>
              ...
              <div className="absolute bottom-7 right-8 hidden h-[92px] w-[92px] items-center justify-center rounded-[21px] border border-[#54bce5]/25 bg-[#15273b]/90 shadow-[0_20px_45px_rgba(0,0,0,0.2)] lg:flex">
                <CalendarDays size={46} strokeWidth={1.5} className="text-[#67bfe6]" />
              </div>
            </section>

            {success && (
              <div className="mt-5 rounded-xl border border-[#bde4cf] bg-[#f1faf5] px-4 py-3 text-sm font-medium text-[#31986d]">
                {success}
              </div>
            )}
              ...
              <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="group relative inline-flex h-10 items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-[#54bce5] via-[#63c9ef] to-[#4f8df7] px-5 text-xs font-semibold text-white shadow-[0_10px_28px_rgba(84,188,229,0.28)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_36px_rgba(84,188,229,0.42)] focus:outline-none focus:ring-4 focus:ring-[#54bce5]/20 disabled:cursor-not-allowed disabled:opacity-60
                    before:absolute before:inset-y-0 before:-left-16 before:w-10 before:rotate-12 before:bg-white/30 before:blur-md before:transition-all before:duration-700 before:content-[''] hover:before:left-[115%]"
                  >
              ...
              {loading ? (
                ...
                <div className="rounded-[22px] border border-dashed border-[#c9d8e5] bg-white px-6 py-16 text-center shadow-[0_7px_22px_rgba(30,60,90,0.04)]">
                  ...
                  <p className="mx-auto mt-2 max-w-md text-[12px] leading-6 text-[#72849a]">
                    Create your first campus event. After creation, students
                    will be able to see it in their Events section.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl bg-[#54bce5] px-5 text-sm font-semibold text-white transition hover:bg-[#3eabd7]"
                  >
                    <Plus size={17} />
                    Create First Event
                  </button>
                    ...
                    return (
                      <article
                        ...
                        style={
                          effect
                            ? {
                                transform: `perspective(1100px) translate3d(${effect.x * 2.5}px, ${effect.y * 2.5 - 4}px, 0) rotateX(${effect.y * -0.75}deg) rotateY(${effect.x * 0.75}deg)`,
                                boxShadow: `0 24px 55px rgba(30, 91, 128, 0.16), ${effect.x * -12}px ${effect.y * -12}px 35px rgba(84, 188, 229, 0.10)`,
                              }
                            : undefined
                        }
                        className="group relative min-w-0 overflow-hidden rounded-[20px] border border-[#d8e3ed] bg-white shadow-[0_7px_22px_rgba(30,60,90,0.05)] transition-[transform,box-shadow,border-color] duration-300 ease-out hover:border-[#7ec9e8] hover:shadow-[0_22px_48px_rgba(30,90,125,0.17)]"
                      >
                        {/* Premium top-level UI accent. It belongs to the card, not the image. */}
                        <div className="pointer-events-none absolute inset-x-0 top-0 z-20 h-[2px] bg-gradient-to-r from-transparent via-[#54bce5] to-transparent opacity-40 transition-all duration-300 group-hover:h-[3px] group-hover:opacity-100" />

                        {/* Cursor-following glow stays above the card UI without changing event images. */}
                        {effect && (
                          <div
                            className="pointer-events-none absolute z-30 h-40 w-40 rounded-full bg-[#54bce5]/10 blur-3xl"
                            style={{
                              left: `calc(${((effect.x + 1) / 2) * 100}% - 80px)`,
                              top: `calc(${((effect.y + 1) / 2) * 100}% - 80px)`,
                            }}
                          />
                        )}

                        <div
                          className={`relative h-[205px] w-full overflow-hidden ${
                            event.image
                              ? "bg-[#dff2fa]"
                              : "bg-gradient-to-br from-[#07111f] via-[#0d1b2d] to-[#152b41]"
                          }`}
                        >
                          {event.image ? (
                            /* Existing event images are intentionally left untouched. */
                            <img
                              src={event.image}
                              alt={event.title}
                            ...
                            <div className="relative flex h-full w-full items-center justify-center overflow-hidden">
                              {/* Dark CampusConnect UI background for events without an image. */}
                              <div
                                className="absolute inset-0 opacity-30"
                                style={{
                                  backgroundImage:
                                    "linear-gradient(rgba(84,188,229,0.12) 1px, transparent 1px), linear-gradient(90deg, rgba(84,188,229,0.12) 1px, transparent 1px)",
                                  backgroundSize: "32px 32px",
                                }}
                              />
                              <div className="absolute -left-12 -top-12 h-36 w-36 rounded-full border border-[#54bce5]/20" />
                              <div className="absolute -right-10 bottom-[-55px] h-44 w-44 rounded-full border border-[#54bce5]/15" />
                              <div className="absolute left-[15%] top-[22%] h-24 w-24 rounded-full bg-[#54bce5]/10 blur-2xl" />
                              <div className="absolute bottom-[10%] right-[18%] h-28 w-28 rounded-full bg-[#4f8df7]/10 blur-2xl" />

                              <div className="relative z-10 flex h-[74px] w-[74px] items-center justify-center rounded-[20px] border border-[#54bce5]/30 bg-[#12263a]/90 text-[#69c9ed] shadow-[0_18px_45px_rgba(0,0,0,0.25)] backdrop-blur-md transition-all duration-500 group-hover:-translate-y-1 group-hover:border-[#69c9ed]/60 group-hover:bg-[#17334b] group-hover:shadow-[0_18px_45px_rgba(84,188,229,0.20)]">
                                <CalendarDays
                                  size={40}
                                  strokeWidth={1.35}
                          ...
                          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#07111f]/55 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />

                          {!event.image && (
                            <div className="pointer-events-none absolute bottom-4 left-4 rounded-full border border-[#69c9ed]/25 bg-[#07111f]/65 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8fdcf6] backdrop-blur-md">
                              CampusConnect Event
                            </div>
                          )}
                        </div>
                        ...
                        <div className="relative z-10 p-5">
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <button
                              ...
                              title="Delete Event"
                              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-red-100 bg-red-50 text-red-500 transition-all duration-200 hover:scale-105 hover:bg-red-100 hover:text-red-600 hover:shadow-[0_8px_18px_rgba(239,68,68,0.12)] disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              {deletingId === event.id ? (
                          ...
                          <h3 className="mt-4 line-clamp-2 font-serif text-[19px] font-bold leading-6 text-[#142238] transition-colors duration-300 group-hover:text-[#1c6085]">
                            {event.title}
                          </h3>
                          ...
                          <div className="mt-4 space-y-2 border-t border-[#edf1f5] pt-4">
                            <div className="flex items-center gap-2 text-[11px] text-[#61768c]">
                              <CalendarDays size={14} className="shrink-0 text-[#54bce5]" />
                              <span>{formatDate(event.eventDate)}</span>
                            </div>
                            <div className="flex items-center gap-2 text-[11px] text-[#61768c]">
                              <Clock size={14} className="shrink-0 text-[#54bce5]" />
                              <span>{formatTime(event.eventDate)}</span>
                            </div>
                            <div className="flex items-center gap-2 text-[11px] text-[#61768c]">
                              <MapPin size={14} className="shrink-0 text-[#54bce5]" />
                              <span className="truncate">{event.venue}</span>
                            </div>
                          </div>

                          <div className="mt-4 flex items-center justify-between rounded-xl bg-[#f7fafc] px-3 py-2 transition-colors duration-300 group-hover:bg-[#eef8fc]">
            ...
            <form onSubmit={handleCreateEvent} className="p-6 sm:p-7">
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <div className="relative">
                    <MapPin size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8ba0b4]" />
                    <input
                      type="text"
                      value={form.venue}
                ...
                <div>
                  ...
                  <div className="relative">
                    <CalendarDays size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8ba0b4]" />
                    <input
                      type="datetime-local"
                ...
                <div>
                  ...
                  {clubs.length === 0 && (
                    <p className="mt-1.5 text-[10px] text-[#9aabba]">
                      No clubs were loaded. Please make sure your existing
                      clubs API is available.
                    </p>
                  )}
                </div>
              ...
              <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <button
                  type="submit"
                  disabled={creating}
                  className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#54bce5] px-6 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(84,188,229,0.2)] transition hover:bg-[#3eabd7] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {creating ? (
                    <>
