...
function Kpi({
  ...
  return (
    <div className="relative overflow-hidden rounded-[20px] border border-[#dbe6ef] bg-white px-5 py-4 shadow-[0_7px_25px_rgba(23,55,80,0.045)]">
      ...
      <p className="mt-3 truncate text-[8px] text-[#8395a7]">{caption}</p>
    </div>
  );
}
...
export default function FacultyDashboardPage() {
  ...
  const [user, setUser] = useState<FacultyUser>(DEFAULT_USER);
  const [stats, setStats] = useState<FacultyStats>(DEFAULT_STATS);
  const [overview, setOverview] = useState<AttendanceOverview>(DEFAULT_OVERVIEW);
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.message || "Unable to load faculty stats.");
      }

      setStats({
        students: Number(data.stats?.students ?? 0),
        pendingApprovals: Number(data.stats?.pendingApprovals ?? 0),
        attendance: Number(data.stats?.attendance ?? 0),
        upcomingEvents: Number(data.stats?.upcomingEvents ?? 0),
      });
    } catch (error) {
  ...
  const loadOverview = useCallback(async (refresh = false) => {
    try {
      ...
      const response = await fetch("/api/faculty/dashboard/overview", {
        method: "GET",
        credentials: "include",
        cache: "no-store",
        headers: {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.message || "Unable to load attendance overview.");
      }

      setOverview(data.attendanceOverview ?? DEFAULT_OVERVIEW);
    } catch (error) {
    ...
    return Array.from(
      new Set(
        overview.subjects
          ...
          .map((item) => normalizeSection(item.section))
          .filter(Boolean),
      ),
    ).sort();
  }, [overview.subjects, semesterFilter]);
      ...
      new Map(
        overview.subjects
          ...
          .filter(
            (item) =>
              sectionFilter === "ALL" ||
              normalizeSection(item.section) === sectionFilter,
          )
          .map((item) => [
            item.subjectId,
            {
  ...
  }, [overview.subjects, semesterFilter, sectionFilter]);
  ...
  const filteredSubjects = useMemo(() => {
    return overview.subjects.filter((item) => {
      const semesterMatch =
        semesterFilter === "ALL" || String(item.semester) === semesterFilter;
      ...
      return semesterMatch && sectionMatch && subjectMatch;
    });
  }, [overview.subjects, semesterFilter, sectionFilter, subjectFilter]);
  ...
  const analytics = useMemo(() => {
    return filteredSubjects.reduce(
      (acc, item) => {
        ...
        acc.expected += item.expectedStudents;
        acc.marked += item.markedStudents;
  ...
  const attendanceHealth =
    analytics.marked > 0
      ? Math.round(
          ((analytics.present + analytics.late + analytics.excused) /
            analytics.marked) *
            100,
        )
      : 0;
  ...
  const markingCoverage =
    analytics.expected > 0
      ? Math.round((analytics.marked / analytics.expected) * 100)
      : filteredSubjects.length
        ? Math.round(
            filteredSubjects.reduce((sum, item) => sum + item.completion, 0) /
              filteredSubjects.length,
          )
        : 0;
  ...
  const pieData = [
    ...
    { name: "Excused", value: analytics.excused },
  ].filter((item) => item.value > 0);

  const subjectChartData = filteredSubjects.slice(0, 8).map((item) => ({
    name:
      item.subjectName.length > 11
        ? `${item.subjectName.slice(0, 11)}…`
        : item.subjectName,
    coverage: item.completion,
    status: item.status,
  }));

  const recentSessions = [...filteredSubjects]
    .sort(
      (a, b) =>
        new Date(b.sessionDate ?? 0).getTime() -
        new Date(a.sessionDate ?? 0).getTime(),
  ...
  const analyticsContext = useMemo(() => {
    if (filteredSubjects.length === 1) {
      const row = filteredSubjects[0];
      return `Semester ${row.semester} • Section ${normalizeSection(
        row.section,
      )} • ${row.subjectName}`;
    }

    if (semesterFilter !== "ALL" && sectionFilter !== "ALL") {
  ...
  return (
    <div className="min-h-screen w-full bg-[#edf4fa] text-[#0d1727]">
      <main className="relative min-h-screen w-full overflow-hidden bg-[#edf4fa] px-4 py-5 sm:px-6 lg:px-8">
        {/* BACKGROUND PATTERN */}
        ...
        <div className="relative w-full">
          {/* =================================================
              HERO (NO "WELCOME BACK", FULL WIDTH)
          ================================================== */}

          <section className="relative w-full overflow-hidden rounded-[28px] border border-[#253d56] bg-gradient-to-br from-[#0b1627] via-[#101e32] to-[#142a40] px-6 py-7 shadow-[0_25px_55px_rgba(7,27,48,0.14)] sm:px-8 lg:px-9">
            ...
            <div className="relative z-10 flex flex-col gap-7 xl:flex-row xl:items-end xl:justify-between">
              <div>
                ...
                <h1 className="font-serif text-[31px] font-bold leading-tight text-[#69caed] sm:text-[42px] lg:text-[47px]">
                  {facultyName}
                </h1>

                <p className="mt-3 max-w-3xl text-[11px] leading-5 text-[#a6b8ca]">
                  One executive workspace for students, approvals, attendance,
                  campus events, activities and clubs.
                </p>
          ...
          <section className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Kpi
              label="Students"
              value={statsLoading ? "..." : String(stats.students)}
            ...
            <Kpi
              label="Attendance"
              value={statsLoading ? "..." : String(stats.attendance)}
            ...
            <Kpi
              label="Events"
              value={statsLoading ? "..." : String(stats.upcomingEvents)}
          ...
          <section className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-[1.65fr_0.75fr]">
            <div className="rounded-[24px] border border-[#d7e3ed] bg-white p-5 shadow-[0_10px_30px_rgba(30,60,90,0.045)] sm:p-6">
              ...
              <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <ExecutiveMetric
                  label="Pending Work"
                  value={String(
                    stats.pendingApprovals + overview.unmarkedSessions,
                  )}
            ...
            <div className="rounded-[24px] border border-[#d7e3ed] bg-white p-5 shadow-[0_10px_30px_rgba(30,60,90,0.045)] sm:p-6">
              ...
              <div className="mt-5 space-y-3">
                ...
                <PriorityRow
                  title="Attendance"
                  value={String(overview.unmarkedSessions)}
                  subtitle="Unmarked sessions"
                  href="/dashboard/faculty/attendance"
                  tone={overview.unmarkedSessions > 0 ? "danger" : "success"}
                />

                <PriorityRow
                  title="Events"
                  value={
                    statsLoading ? "..." : String(stats.upcomingEvents)
                  }
          ...
          <section className="mt-5 overflow-hidden rounded-[25px] border border-[#d7e3ed] bg-white shadow-[0_10px_30px_rgba(30,60,90,0.05)]">
            ...
            <div className="grid grid-cols-1 items-stretch xl:grid-cols-[minmax(0,1fr)_360px]">
              {/* BAR */}
              <div className="border-b border-[#e8eef3] p-5 sm:p-6 xl:border-b-0 xl:border-r">
                <div className="mb-3">
                  ...
                  <p className="mt-1 text-[9px] text-[#8395a7]">
                    Green indicates marked sessions. Red indicates unmarked sessions.
                  </p>
                </div>
          ...
          <section className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-[1.1fr_0.9fr]">
                  ...
                  recentSessions.map((item) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        >
                          {item.status === "MARKED" ? "MARKED" : "NOT MARKED"}
                        </p>
                      </div>
                    </div>
                  ))
            ...
            <div className="rounded-[25px] border border-[#d7e3ed] bg-white p-5 shadow-[0_10px_30px_rgba(30,60,90,0.05)] sm:p-6">
              ...
              <div className="mt-5 grid grid-cols-1 gap-2.5 sm:grid-cols-3 xl:grid-cols-1">
                <ProfileItem label="Faculty ID" value={facultyId} />
                <ProfileItem label="Role" value={facultyRole} />
                <ProfileItem
            ...
            <section className="group relative flex h-full min-h-[300px] flex-col overflow-hidden rounded-[25px] border border-[#203c54] bg-gradient-to-br from-[#0a1727] via-[#0e2033] to-[#12314a] p-5 text-white shadow-[0_18px_45px_rgba(7,27,48,0.12)] sm:p-6">
              ...
              <div className="relative mt-4 flex items-end gap-1.5">
                {[42, 68, 52, 84, 61, 92, 72].map((height, index) => (
                  <div
                    key={`event-signal-${index}`}
                    className="flex-1 rounded-t-md bg-[#54bce5]/20 transition duration-500 group-hover:bg-[#54bce5]/35"
                    style={{ height: `${height}px` }}
                  >
                    <div
                      className="h-full origin-bottom rounded-t-md bg-[#54bce5]/60 transition duration-700 group-hover:scale-y-100"
                      style={{
                        transform: `scaleY(${0.35 + index * 0.07})`,
                      }}
                    />
                  </div>
                ))}
...
function ExecutiveMetric({
  ...
  return (
    <div className="rounded-[19px] border border-[#e2eaf0] bg-[#fbfdff] p-4">
      ...
      <p className="mt-1 truncate text-[8px] text-[#8496a7]">{caption}</p>
    </div>
  );
}
...
function PriorityRow({
  ...
  const colors = {
    danger: "bg-[#fff0f0] text-[#d95757]",
    warning: "bg-[#fff7e8] text-[#d39434]",
    success: "bg-[#eefaf5] text-[#2d986c]",
    info: "bg-[#edf8fd] text-[#3b92bb]",
  }[tone];
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-bold text-[#30485e]">{title}</p>
        <p className="mt-1 text-[8px] text-[#8295a7]">{subtitle}</p>
      </div>

      <p className="font-serif text-[21px] font-bold text-[#22394e]">{value}</p>
    </Link>
  );
}
...
function LegendRow({
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-[#edf1f4] bg-[#fbfdff] px-3 py-2.5">
      ...
      <span className="text-[9px] font-bold text-[#2b4359]">{value}</span>
    </div>
  );
}
...
function ProfileItem({
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-[#edf2f5] bg-[#fbfdff] px-3.5 py-2.5">
      <span className="text-[8px] text-[#7c90a1]">{label}</span>

      <span
        className={`text-[8px] font-bold ${
          positive ? "text-[#2d986c]" : "text-[#304a61]"
        }`}
      >
        {value}
      </span>
...
function LoadingState() {
  return (
    <div className="flex min-h-[230px] items-center justify-center text-[10px] font-semibold text-[#7c90a1]">
      <RefreshCw size={17} className="mr-2 animate-spin text-[#54bce5]" />
      Loading live data...
    </div>
  );
}
