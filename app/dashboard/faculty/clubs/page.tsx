...
function getMemberCount(club: Club) {
  if (
    typeof club.memberCount === "number" &&
    Number.isFinite(club.memberCount)
  ) {
...
export default function FacultyClubsPage() {
  ...
  const [faculty, setFaculty] = useState<FacultyUser>(defaultFaculty);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        console.error("FACULTY LOAD ERROR:", loadError);
      }
    }, 0);
  ...
  const facultyId = faculty.facultyId || "FACULTY";

  /* ==================================================
     FETCH CLUBS
  ================================================== */

  const fetchClubs = useCallback(async (showRefresh = false) => {
    try {
      if (showRefresh) {
        setRefreshing(true);
      }

      const response = await fetch("/api/faculty/clubs", {
        method: "GET",
        credentials: "include",
        cache: "no-store",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(
          data?.message || `Failed to fetch clubs (${response.status}).`
        );
      }

      const receivedClubs = Array.isArray(data?.clubs)
        ? data.clubs
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          console.error("FACULTY FETCH CLUBS ERROR:", fetchError);
  ...
  const totalMembers = useMemo(() => {
    return clubs.reduce(
      (total, club) => total + getMemberCount(club),
      0
    );
  }, [clubs]);
  ...
  function updateForm(field: keyof ClubForm, value: string) {
    setForm((current) => ({
      ...current,
      [field]: value,
    }));
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.message ||
            (isEditing ? "Failed to update club." : "Failed to create club.")
        );
      }

      setSuccess(
        isEditing ? "Club updated successfully." : "Club created successfully."
      );

      closeModal();
      await fetchClubs();
    } catch (submitError) {
      console.error("FACULTY CLUB SUBMIT ERROR:", submitError);
  ...
  async function handleDelete(club: Club) {
    ...
    try {
      ...
      const response = await fetch(`/api/faculty/clubs/${club.id}`, {
        method: "DELETE",
        credentials: "include",
        headers: {
          Accept: "application/json",
      ...
      console.error("FACULTY DELETE CLUB ERROR:", deleteError);
  ...
  function handleImageError(event: React.SyntheticEvent<HTMLImageElement>) {
    ...
    if (parent) {
      parent.classList.add("club-image-fallback");
    }
  }
            ...
            <section className="group/hero relative flex h-[280px] w-full items-center justify-between overflow-hidden rounded-[24px] border border-[#263951] bg-gradient-to-br from-[#0d1728] via-[#101d30] to-[#14273b] px-6 text-white shadow-[0_18px_45px_rgba(10,27,48,0.18)] transition-all duration-500 hover:shadow-[0_25px_60px_rgba(10,27,48,0.22)] sm:px-9 lg:px-11">
              ...
              <div className="relative z-10 max-w-[850px]">
                ...
                <h1 className="font-serif text-[34px] font-bold leading-[1.03] tracking-[-0.03em] text-white sm:text-[43px] lg:text-[50px]">
                  Manage campus
                  <br />
                  <span className="text-[#69c9ed]">clubs with ease.</span>
                </h1>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-[#0b1423]">
                          <div className="text-center text-white">
                            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-[#54bce5]/30 bg-[#54bce5]/10">
                              <ImageIcon size={31} className="text-[#67bfe6]" />
                            </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={form.logo}
                      onChange={(event) =>
                        updateForm("logo", event.target.value)
                      }
                      placeholder="https://..."
                      disabled={saving}
                      className="w-full rounded-lg border border-[#d8e3ed] px-3 py-2.5 text-sm text-[#203149] outline-none transition focus:border-[#54bce5] focus:ring-4 focus:ring-[#54bce5]/10 disabled:bg-[#f7fafc]"
                    ...
                    {form.logo ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center text-white/70">
                          ...
                          />
                          <p className="mt-3 text-sm">Club Image Preview</p>
                        </div>
                      </div>
                    )}
